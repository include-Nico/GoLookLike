...
function generateRandomItem() {
  ...
  do {
    ...
    item = { id: 'item_' + Date.now() + Math.floor(Math.random()*1000), hash: itemHash, emoji: tipo.emoji, pos: tipo.pos, nome_capo: `${tipo.nome} ${colore.nome}`, brand: 'Generico', colore_hex: colore.hex, pesantezza: tipo.pesantezza, contesti: contestiShuffled.slice(0, 2), quantity: 1, is_dirty: false, is_favorite: false };
    attempts++;
...
async function fetchRealWeather(lat, lon, isSilent = false) {
  try {
    ...
    if (!res.ok) throw new Error("Chiamata fallita");
...
function updateProfileView() {
  ...
  document.getElementById('profile-name').innerText = userName; document.getElementById('profile-email').innerText = userEmail;
  const dirtyCount = appState.wardrobe.filter(i => i.is_dirty).length; document.getElementById('laundry-count-badge').innerText = dirtyCount;
}
...
function getCleanWardrobe() { const clean = appState.wardrobe.filter(i => !i.is_dirty); return clean.length > 5 ? clean : [...clean, ...Array.from({length: 15}).map(generateRandomItem)]; }
...
function renderOutfitStack() {
  ...
  ['top', 'bottom', 'shoes', 'accessory'].forEach(pos => {
    ...
    h.on('panend', (ev) => {
      if (Math.abs(ev.deltaX) > 80) { el.style.transform = `translateX(${ev.deltaX > 0 ? '100%' : '-100%'})`; el.style.opacity = '0'; setTimeout(() => swapSingleItem(pos), 300); }
    });
  });
}
