/**
 * GoLookLike - Security & Session Management
 */
...
async function verifyInitialSession() {
  ...
  try {
    ...
    if (res.status === 'success') {
      console.log(`Bentornato, ${res.data.email}`);
      initCoreApp(); // Avvia l'app bypassando il login
    } else {
...
document.getElementById('btn-send-otp').addEventListener('click', async () => {
  ...
  try {
    ...
    document.getElementById('otp-email-display').innerText = email; // Aggiunto nell'HTML per UX
    navigateTo('view-otp');
  } catch (error) {
    alert("Errore invio OTP.");
  } finally {
    btn.innerText = "Ricevi Codice OTP";
    btn.disabled = false;
  }
});
...
document.getElementById('btn-verify-otp').addEventListener('click', async () => {
  ...
  try {
    ...
    btn.innerText = "Entra nell'Armadio";
    btn.disabled = false;
  }
});

// Funzione di simulazione per sviluppo rapido locale
function mockApiCall(action, payload) {
  return new Promise((resolve) => {
    setTimeout(() => {
      if (action === 'checkSession') resolve({ status: 'error' }); // Forza il login al primo avvio mockato
